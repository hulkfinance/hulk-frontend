...
const Referral: React.FC = () => {
  ...
  return (
    <>
      <ReferralLinkInput type='text' ref={referralLink} onChange={event => setReferralLinkValue(event.target.value)}
                         value={referralLinkValue} readOnly />
      <Banner>
        <BannerHeading as='h1' mb='0' color='secondary'>HULK Referral Program</BannerHeading>
        <BannerText color='primary'>Share the referral link below to invite your friends and earn 1% of your friends
          earnings FOREVER!</BannerText>
      </Banner>
      <Page>
        <div>
          <Cards>
            <TotalReferrals>
              <CardBody p={40}>
                <CardHeading mb='24px'>Total Referrals</CardHeading>
                <CardValue>0</CardValue>
              </CardBody>
            </TotalReferrals>
            <TotalReferrals>
              <CardBody p={40}>
                <CardHeading mb='24px'>Total Referral Commissions</CardHeading>
            ...
            <CardReferralLink>
              <CardBody p={40}>
                <ReferralHeading>
                  <CardHeading>Your Referral Link</CardHeading>
                  {
                    account && <Button onClick={copyToClipboard} variant='secondary'
                                       size='sm'>{isCopiedLink ? 'Copied' : 'Copy'}</Button>
                  }
                </ReferralHeading>
                {
                  account
                    ? <ReferralLink
                      color='primary'>{referralLinkValue}</ReferralLink>
                    : <UnlockButton />
                }
              </CardBody>
            </CardReferralLink>
