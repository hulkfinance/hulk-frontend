...
function Swap() {
  ...
  const {
    allowance,
    hulkBalance,
    hulkPreBalance, pendingApprove, pending, onApprove, onSwap, amountOut, onChangeAmountIn, onChangeAmountOut, amount,
  } = useHulkSwap()
  ...
  return (
    <SwapStyledCard>
      ...
      <CardBody p={40}>
        ...
        <SwapText><span>HULK token</span> will be distributed block by block from Feb 4th, 2022 ( 30% each
          month)</SwapText>
        <SwapBlock>
          <Amount>
            <InputBlock>
              ...
              <AmountInput type='text' scale='md'
                           value={amount}
          ...
          {account ?
            allowance.isZero()
              ? <Button variant='primary' onClick={onApprove} fullWidth
                        disabled={pendingApprove}>{pendingApprove ? 'Wait' : 'Approve'}</Button>
              : <Button onClick={onSwap} fullWidth
                        disabled={isDisabled}
                        variant='primary'>{pending ? 'Wait' : 'Swap hulk'}</Button>
